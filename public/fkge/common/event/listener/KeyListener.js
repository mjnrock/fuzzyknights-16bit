...
class KeyListener {
	...
	OnKeyUp(e) {
		...
		this.FuzzyKnights.Common.Event.Spawn.InputPlayerKeyStateEvent(this.PlayerKeyState);
		this.FuzzyKnights.Common.Event.Spawn.InputKeyboardEvent(e);
	}
	...
	FlagController(eventKey, isAdd = true) {
		for(let key in this.FuzzyKnights.Common.Game.Settings.Bindings.Movement) {
			if(this.FuzzyKnights.Common.Game.Settings.Bindings.Movement[key].includes(eventKey)) {
				if(!!isAdd) {
