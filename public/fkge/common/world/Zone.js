...
class Zone {
	...
	PlaceEntity(entity, x, y) {
		Zone.FuzzyKnights.Common.Event.Spawn.EntityMoveEvent(this, entity, -1, -1, x, y);
	...
	Move(entity, x0, y0, x1, y1, isDeltaMove = false) {
		...
		Zone.FuzzyKnights.Common.Event.Spawn.EntityMoveEvent(this, entity, x0, y0, x1, y1);
	...
	ApplyPhysics(entity, pos) {
		...
		if(terrain) {
			let veloc = Zone.FuzzyKnights.Common.Component.Mutator.Physics.GetVelocity(entity),
				vt = Zone.FuzzyKnights.Common.Component.Mutator.TerrainInfo.GetNavigabilityConstraint(terrain);

			// console.log(veloc.X, veloc.Y, vt * Zone.FuzzyKnights.Common.Game.GameManager.GameLoop.LastTimeStep / 10000);
			// veloc.X.Subtract(vt * Zone.FuzzyKnights.Common.Game.GameManager.GameLoop.LastTimeStep / 10000);
			// veloc.Y.Subtract(vt * Zone.FuzzyKnights.Common.Game.GameManager.GameLoop.LastTimeStep / 10000);

			// Zone.FuzzyKnights.Common.Component.Mutator.Physics.SetVelocity(entity, Zone.FuzzyKnights.Common.Physics.D2.Velocity.Generate(dvx, dvy));
			// Zone.FuzzyKnights.Common.Component.Mutator.Physics.AddForce(entity, Zone.FuzzyKnights.Common.Physics.D2.Force.Generate(-Fx, -Fy));
			// Zone.FuzzyKnights.Common.Component.Mutator.Physics.GetAcceleration(entity).Merge(Zone.FuzzyKnights.Common.Physics.D2.Acceleration.Generate(dvx, dvy));
		}
	}

	//? @protagonist is a Player's Entity
	Tick(time, protagonist) {
		let protPos = Zone.FuzzyKnights.Common.Component.Mutator.Worlds.GetPoint(protagonist),
			dist = 5;
